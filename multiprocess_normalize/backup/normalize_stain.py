...
def calculate_source_matrices(req_queue, out_queue, pwsi_gen, save_dir, lk, total_ROIs, msg, msg_fmt, target_RM_H, target_W):
    ...
    while True:
        ...
        csv_path_vec1_6_thres4_sat25 = make_csv(save_dir, slide_name, origin_name, csv_name_vec1_6_thres4_sat25, column_flag, lk)
        ...
        try:
            ...
            msg = msg_fmt.format(left=total_ROIs - out_queue.qsize(), total=total_ROIs)
            ...
            output_H(source_ROI, norm_H, slide_name, origin_name, save_dir,
                     csv_path_vec1_6_thres4_sat25, vec_thres1_6, thres_4_0, sat_thres_25, flag_num, column_flag)
        ...
        for source_slide in tqdm(source_list, ascii=True):
           ...
           # プロセスの初期駆動
            for _ in range(max_workers):
                p = mp.Process(target=calculate_source_matrices,args=(req_queue, out_queue,
                 pwsi_gen, save_dir, lk, total_ROIs, msg, msg_fmt, target_RM_H, target_W))
                p.start()
...
#                source_ROI, source_H, source_RM_H = out_queue.get()  # とり出せるまでブロックする(デフォルト)
#                print("get from out_queue. left_qsize : %d.", out_queue.qsize())
#                norm_H = source_H * (target_RM_H / source_RM_H)
#                slide_name = pwsi_gen.get_slide_name(source_ROI)
#                origin_name = pwsi_gen.get_origin(slide_name)
#                normalize(source_ROI, norm_H, target_W, 
#                          slide_name, origin_name, save_dir)
#
#                # 正規化が終わったら残ROI数を減らす
#                left_ROIs -= 1
#                print("normalize finished. left_ROIs : %d", left_ROIs)
#                if msg:
#                    sys.stdout.write('\r' + (' ' * len(msg)))
#
#                msg = msg_fmt.format(left=total_ROIs - left_ROIs, total=total_ROIs)
#                sys.stdout.write('\r' + msg)
#
